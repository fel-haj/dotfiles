{config, pkgs, ...}: {
  ...
  home.shellAliases = {
    darwin = "darwin-rebuild --flake ${config.home.homeDirectory}/Development/dotfiles/nix";

    ls = "eza";
    lt = "eza --tree --level=2";

    set_env = "source ${config.home.homeDirectory}/Development/util/scripts/sh/set_env.sh -p";
  };
}
